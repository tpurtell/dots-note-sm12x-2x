...
docker run -d --name "$container" --gpus all --ipc=host --network=host \
  ...
  -e DOTS3_ALLOCATOR_FRACTION="${DOTS3_ALLOCATOR_FRACTION:-}" \
  -e PYTORCH_ALLOC_CONF="${PYTORCH_ALLOC_CONF:-}" \
  -e VLLM_HYBRID_LAYER_PARTITION="${VLLM_HYBRID_LAYER_PARTITION:-}" \
  -e VLLM_HYBRID_BALANCE_KV_GROUPS="${VLLM_HYBRID_BALANCE_KV_GROUPS:-0}" \
  -e VLLM_HYBRID_PACKED_ROUTING="${VLLM_HYBRID_PACKED_ROUTING:-0}" \
  -e VLLM_HYBRID_OVERLAP_SHARED="${VLLM_HYBRID_OVERLAP_SHARED:-0}" \
