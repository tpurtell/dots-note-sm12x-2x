...
def main() -> None:
    ...
    warmup = exact_prompt(args.base_url, args.model, 512, f"{run_nonce}-warmup")
    time_to_first_token(args.base_url, args.model, warmup)

    points = []
    for target in args.prompt_tokens:
        depth_warmup = exact_prompt(
            args.base_url, args.model, target, f"{run_nonce}-{target}-warmup"
        )
        time_to_first_token(args.base_url, args.model, depth_warmup)
        runs = []
        for run in range(args.runs):
            prompt = exact_prompt(
                args.base_url, args.model, target, f"{run_nonce}-{target}-{run}"
            )
            ...
            runs.append(result)
            print(
                f"Dots3 {target:,} run {run + 1}/{args.runs}: "
    ...
    report = {
        ...
        "model": args.model,
        "quant_profile": "fp8-core-exl3-k4-routed",
        "kv_cache_dtype": "fp8",
