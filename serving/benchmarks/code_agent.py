#!/usr/bin/env python3
"""C1 reference coding task at existing KV depths, with native Dots3 chat rendering."""
# Adapted from the Qwen3.8 reference recipe; prompt and timing conventions retained.
import argparse
import hashlib
...
spec = importlib.util.spec_from_file_location("context_bench", Path(__file__).with_name("context.py"))
...
parser.add_argument("--depths", type=int, nargs="+", default=[0,8192,24000])
...
with args.output.open("x") as output:
    output.write(json.dumps({"record": "meta", "schema": "dots3-code-agent-depth-v1",
        ...
        "method": "Reference async task-runner coding task; Dots3 non-thinking rendering; filler before rendered task; repeated prompts retain prefix caching; decode excludes TTFT; forced length is not a code-correctness test"}) + "\n")
