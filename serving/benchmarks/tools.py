...
def main():
    ...
    with args.output.open("x") as output:
        output.write(json.dumps({"record": "meta", "schema": "dots3-api-tool-constraints-v1",
            ...
            for mode in ("required", "named", "auto", "none"):
                ...
                for call in calls:
                    try:
                        ...
                        correct = False
                row = {"record": "measurement", "streaming": streaming,
                       "mode": mode, "passed": correct, "request": payload,
                       "calls": calls, "raw_response": raw}
                cases.append(row)
                output.write(json.dumps(row) + "\n")
                output.flush()
                print(json.dumps({k: row[k] for k in ("streaming", "mode", "passed")}), flush=True)
