...
### Pending Spark retrieval after client timeout

`continue_spark_timeout.py` preserves the original manifest, all sixteen completed stage receipts, and the failed metadata-only near-maximum retrieval attempt. It requires unchanged container/guard identities and frozen sources. Its isolated child executes the original retrieval module with one audited in-memory change: the workloads HTTP read timeout increases from 900 to 3600 seconds. Prompts, output budget, timing and validators are unchanged. The completion/report include `timeout_continuation` with original/effective source hashes and preserved-file hashes; the failure is archived, not erased. A source with any completed retrieval measurement is rejected to prevent repeated work. This does not restart or reconfigure serving containers.
