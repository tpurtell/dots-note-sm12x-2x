...
raw=json.dumps({'passed':True,'workers':[{'rank':i,'world_size':2,'passed':True,'errors':[],
    'layers':[{'layer':n,'owner_rank':int(n>=23),'local_owner':int(n>=23)==i} for n in range(46)],
    'multimodal_owner_plan':{'visual':0,'audio_tower':1},
    'cache_admission_inputs':{'max_model_len':524288,'max_num_batched_tokens':512}} for i in range(2)]})
...
head={'image_id':'sha256:test','args':args,
      'selected_environment':['VLLM_HYBRID_LAYER_PARTITION=23,23','VLLM_HYBRID_MM_OWNERS=0,1','VLLM_HYBRID_ATTESTATION_PATH=/cache/owners.json'],
...
for failure in ['missing-head','missing-rank','tamper','image','profile','duplicate-head','wrong-cut','wrong-mm','wrong-context','wrong-batch']:
    ...
    if failure in ('wrong-cut','wrong-mm','wrong-context','wrong-batch'):
        proof=test['rhea']['hybrid_attestation'];data=json.loads(proof['raw_json'])
        if failure=='wrong-cut':data['workers'][0]['layers'][22]['owner_rank']=1
        if failure=='wrong-mm':data['workers'][1]['multimodal_owner_plan']={'visual':1,'audio_tower':0}
        if failure=='wrong-context':data['workers'][1]['cache_admission_inputs']['max_model_len']=262144
        if failure=='wrong-batch':data['workers'][1]['cache_admission_inputs']['max_num_batched_tokens']=256
        proof['raw_json']=json.dumps(data);proof['sha256']=hashlib.sha256(proof['raw_json'].encode()).hexdigest()
    try:spark_hybrid_attestations(test)
    except ValueError:pass
    else:raise AssertionError('accepted '+failure)
print('PASS: centralized head receipt/worker reference; missing head/rank, hash tamper, image/profile drift, duplicate receipt, wrong cut/MM/context/batch rejected')
