...
Projection checkpoints are enabled on the coordinator and worker. Durable subset Hessian captures are enabled on the coordinator, so a restarted run can reuse a completed subset without replaying it. A durable activation layer boundary is still needed to resume directly at the next decoder layer after a coordinator failure; do not remove run state or source checkpoints while the quant is active.
