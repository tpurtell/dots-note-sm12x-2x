#!/usr/bin/env python3
"""Build source-disjoint Dots3 Note EXL3 calibration and held-out corpora."""

# Adapted from glmrt/python/tools/build_glm52_calibration_corpus.py. The source
# selection is preserved; prompt rendering and tokenizer identity are Dots3 Note.
...
# The Dots3 Note chat template's single-user, non-thinking, no-tools form.
# The JSONL stores the unwrapped prompt. This form determines token budgets.
_SYSTEM = "<|system|>You are a helpful assistant.<|endofsystem|>"
_USER = "<|user|>"
_END_USER = "<|endofuser|>"
...
def render_nonthinking_messages(messages: list[dict[str, Any]]) -> str:
    ...
    return f"{_SYSTEM}{_USER}{content}<no_think>{_END_USER}{_ASSISTANT}"


SCHEMA = "dots3-note-exl3-calibration-corpus-v1"
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    builder_repo = builder_path.parents[1]
