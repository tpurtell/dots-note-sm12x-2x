#!/usr/bin/env python3
"""Crash-consistent rolling decoder-layer boundaries for Dots3 Note EXL3.

Adapted from the GLMRT layer-boundary store for Dots3's stateless decoder
kwargs and its uniform routed-expert projection coverage.
"""
...
BOUNDARY_SCHEMA = "dots3.exl3-layer-boundary"
...
REPLAY_STATE_TENSOR_FIELDS: tuple[str, ...] = ()
...
class Dots3LayerBoundaryStore:
    ...
    def discover_completed_projection_layers(
        ...
        journal = _read_error_journal(self.error_journal_path)
        mixed = self.family_join.get("inline_mixed")
        candidate_journal: set[str] = set()
        if isinstance(mixed, dict):
            base_bits = mixed["base_bits"]
            candidate_path = self.error_journal_path.with_name(
                f"{self.error_journal_path.name}.k{base_bits}-candidates"
            )
            if candidate_path.exists() or candidate_path.is_symlink():
                candidate_journal = _read_error_journal(candidate_path)
        ...
        for request, result in inspected:
            ...
            candidate_only = (
                ...
                and inline.get("base_bits") == mixed["base_bits"]
                and inline.get("role") == f"candidate_k{mixed['base_bits']}"
                and contract.get("bits") == mixed["base_bits"]
                and record_sha256 in candidate_journal
                and record_sha256 not in journal
            )
            if (
                ...
                or not isinstance(provenance, dict)
                or provenance.get("family_join") != self.family_join
                or (record_sha256 not in journal and not candidate_only)
            ):
                raise LayerBoundaryError(
                    f"projection discovery evidence differs for {module_name}"
                )
            # Low-tier candidates are authenticated recovery inputs, not final
            # layer outputs. Only the final journal selects installable records.
            if candidate_only:
                continue
            entry = {
                "module": module_name,
    ...
    def load_latest(
        ...
        *,
        ...
        if (
            ...
            or len(replay_state_shards)
            != (len(shards) if REPLAY_STATE_TENSOR_FIELDS else 0)
        ):
        ...
        restored_state: list[dict[str, torch.Tensor]] = (
            [{} for _ in shards] if not REPLAY_STATE_TENSOR_FIELDS else []
        )
        ...
        *,
        ...
        try:
            ...
            for batch_index, kwargs in enumerate(layer_input_kwargs):
                ...
                if not REPLAY_STATE_TENSOR_FIELDS:
                    continue
...
class Dots3LayerBoundaryController:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(store, Dots3LayerBoundaryStore):
    ...
    def _processor(processors: list[Any]) -> Any:
        ...
        if len(candidates) != 1:
            raise LayerBoundaryError(
                "Dots3 Note layer-boundary resume requires exactly one "
