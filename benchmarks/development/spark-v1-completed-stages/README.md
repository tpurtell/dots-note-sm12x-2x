...
This is a partial evidence archive, **not an accepted release qualification**. It preserves only the already validated seven-workload, sampled-code, and concurrent-client stages on the published Spark image, plus the explicitly inherited prefix/API/multimodal receipts. Active coding and unfinished stages were not read or archived.
...
[manifest.json](manifest.json) lists SHA-256 hashes for all 72 original and losslessly compressed artifacts. [summary.json](summary.json) contains the extracted metrics and exact identities. The archive does not activate the Spark release profile; unfinished qualification must still complete.
