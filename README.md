...
**Status (2026-09-25):** quantization and the complete tensor audit passed. The checkpoint is published at revision `d8e3b9a48d3b5b8e23d9c6b3f6cc645f48b2f9da`; its 36 artifact files total 163,552,088,967 bytes. RTX development checks pass text, image, audio, prefix caching, constrained JSON, eight tool-call cases, and the seven content contracts. Full performance tuning, final-image qualification, and GHCR publication remain pending. Spark and RTX serving recipes are being developed in parallel; Spark work currently awaits host reboot recovery. See the [serving qualification ledger](docs/serving-progress.md) for remaining gates, including separate GHCR releases.
